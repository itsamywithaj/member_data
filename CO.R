...
# ----- racial demographics -----
# Downloaded from https://www.cde.state.co.us/cdereval/2022-2023schoolmembershipethnicityracegender

race_enrl <- read_excel("raw data/CO_2022-23_Membership_Race_Gender_byGradeSchool.xlsx",sheet = "Data")
str(race_enrl)
...
#----- FRPL ----
# FRL downloaded from https://www.cde.state.co.us/cdereval/2022-2023schoolpk-12frl
frpl <- read_excel("raw data/CO_2022-23_PK12_FRL_bySchool - Suppressed.xlsx", sheet = "Data")
...
# Downloaded from https://www.cde.state.co.us/cdereval/2022-2023schoolipst 
ellsped <- read_excel("raw data/CO_2022-23_IPST_bySchool - Suppressed.xlsx",sheet = "Data")
...
# ---- CMAS RESULTS -------
# Downloaded
acad <- read_excel('raw data/CO_2022 CMAS ELA and Math District and School Summary Achievement Results.xlsx')
names(acad) <- tolower(acad[12,])
str(acad)
acad <- acad[-(1:12),]
levels(as.factor(acad$content))
memb_acad <- acad %>% 
  mutate(district = as.factor(`district name`),
         school = as.factor(`school name`),
         test = as.factor(content),
         n_tests = as.numeric(gsub("[^0-9.-]", "",`number of valid scores`)),
         p_prof = as.numeric(`2022`) * .01) %>% 
  mutate_if(is.numeric, ~replace_na(., 0)) %>% 
  select(district:p_prof, grade) %>% 
  filter(str_detect(school, "DSST")|
           str_detect(school, "Downtown Denver"),
         grade == "All Grades") %>% 
  select(-grade)
levels(memb_acad$test)[1:2] <- c("ela","math")
...
dist_acad <- acad %>% 
  ...
  filter(level == "DISTRICT",
         ...
         test == "English Language Arts"|
           test == "Mathematics") %>% 
  select(district, test, p_prof) %>% 
  spread(test, p_prof)
names(dist_acad)[2:3] <- c("dist_ela","dist_math")

memb_acad <- merge(dist_acad,memb_acad) %>% select(school, district, math, dist_math, ela, dist_ela)

write.csv(memb_acad, file = file.path('output data/co_acad.csv'), row.names = FALSE) 
# -- END -----
